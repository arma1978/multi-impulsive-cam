...
int main( void )
{
   ...
   // order = 1;
    ...
    infile.open("runtime/xd0J2.dat"); // debris initial state
	if (infile.is_open())
	{
		infile >> xdum[0];
		infile >> xdum[1];
   ...
   for (int j = 3; j<6 ; j++) {
    ...
    xdfJ2 = RK78(6, xd0, 0.0, cons(tfc), rhsp);
    xdfJ2  = propJ2An(xdfJ2, dt, muSc, rE/Lsc, J2); // time dependence of final state on time
    
    //xdfJ2  = propJ2An(xd0, tfc, muSc, rE/Lsc, J2);
